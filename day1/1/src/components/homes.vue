...
export default {
    ...
    methods: {
        ssmc() {
            if (this.model != '' && this.input != '' && this.input1 != '') {
                let a = this.tableData2.filter(ele => {
        ...
        xr() {
            this.$axios.get('/list.json').then(res => {
                ...
                this.tableData2 = this.tableData2.filter(ele => {
                    return ele.created_at = new Date(parseInt(Number(ele.created_at)) * 1000).toLocaleString().replace(/:\d{1,2}$/, ' ');
                })
                this.tableData2 = this.tableData2.filter(ele => {
                    if (ele.status == 1) {
